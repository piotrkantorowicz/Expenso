...
internal sealed class HandleAsync : IntegrationEventHandlerLoggingDecoratorTestBase
{
    ...
    public async Task Should_LogInformation_When_NoExceptionOccurred()
    {
        // Arrange
        // Act
        await TestCandidate.HandleAsync(_testIntegrationEvent, It.IsAny<CancellationToken>());

        // Assert
        _loggerMock.Verify(
            x => x.Log(LogLevel.Information, LoggingUtils.IntegrationEventExecuting,
                It.Is<It.IsAnyType>((v, t) => true), It.IsAny<Exception>(),
                It.IsAny<Func<It.IsAnyType, Exception, string>>()!), Times.Once);

        _loggerMock.Verify(
            x => x.Log(LogLevel.Information, LoggingUtils.IntegrationEventExecuted, It.Is<It.IsAnyType>((v, t) => true),
    ...
    public void Should_LogError_When_ExceptionOccurred()
    {
        ...
        _integrationEventHandlerMock
            .Setup(x => x.HandleAsync(_testIntegrationEvent, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new Exception("Intentional thrown to test error logging"));

        // Act
        Exception? exception = Assert.ThrowsAsync<Exception>(() =>
            TestCandidate.HandleAsync(_testIntegrationEvent, CancellationToken.None));
