    ...
    : INotificationServiceFactory
{
    private readonly IDictionary<string, INotificationService> _servicesDictionary =
        servicesDictionary ?? throw new ArgumentNullException(nameof(servicesDictionary));
    ...
    public T GetService<T>() where T : INotificationService
    {
        ...
        {
            throw new InvalidOperationException($"Notification service is not of requested type {typeof(T).FullName}.");
        }
