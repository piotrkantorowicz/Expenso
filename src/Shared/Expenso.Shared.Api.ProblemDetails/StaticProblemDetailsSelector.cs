...
public static class StaticProblemDetailsSelector
{
    ...
    {
        if (!ProblemDetailsMap.TryGetValue(key: (ProblemStatusCodes)statusCode,
                value: out BaseProblemDetails? problemDetails))
        {
            throw new ArgumentOutOfRangeException(paramName: nameof(statusCode),
                message: $"The status code {statusCode} is not supported.");
        }
            ...
            case BaseTypedProblemDetails<string> problem:
                {
                    if (detail is not string detailString)
                    {
                        throw new InvalidOperationException(
                            message: $"The problem details type {problemDetails.GetType().Name} is not supported.");
                    }
