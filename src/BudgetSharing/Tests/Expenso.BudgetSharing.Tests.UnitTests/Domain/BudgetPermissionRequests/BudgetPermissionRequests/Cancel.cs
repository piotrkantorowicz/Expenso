...
internal sealed class Cancel : BudgetPermissionRequestTestBase
{
    ...
    public void Should_MarkBudgetPermissionRequestAsCancelled()
    {
        ...
        // Assert
        TestCandidate.StatusTracker.Status.ShouldBe(expected: BudgetPermissionRequestStatus.Cancelled);
    ...
    public void Should_ThrowDomainRuleValidationException_When_BudgetPermissionRequestHasBeenAlreadyCancelled()
    {
        ...
        action.AssertDomainRuleValidationException(
            expectedDetails: $"Only pending budget permission request {TestCandidate.Id} can be made cancelled.");
    }
